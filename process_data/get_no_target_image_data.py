...
def main(file_name, root_dir, dir_name, fs_dir, max_workers=3, test_ratio=0.2):
    # 查看照片中所有的年龄和性别的标注情况
    # 查看数据所有的年龄和性别标注情况
    no_target_action = read_images_json_file(file_name)
    no_target_image_download_dir = os.path.join(root_dir, dir_name)
    print(f"no_target image download dir: {no_target_image_download_dir}")
    # 检查目录是否存在
    if os.path.exists(no_target_image_download_dir):
        # 如果存在，删除目录
        shutil.rmtree(no_target_image_download_dir)
        print(f"目录 {no_target_image_download_dir} 已删除")
    # 创建目录
    os.makedirs(no_target_image_download_dir)
    print(f"目录 {no_target_image_download_dir} 已创建")
    # 拉取 对应的图片数据
    get_image_train_test_data_to_local_dir(no_target_action, dir_name, fs_dir, root_dir, test_ratio, max_workers)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-j', '--json_name', type=str,
                        default="/home/yuzhong/nndata/export/20240428/空白图片1.json", help='下载的图片的json文件路径')
    parser.add_argument('-r', '--root_dir', type=str,
                        default="/mnt/data2/all_species_240916", help='该数据下载的根目录名字')
    parser.add_argument('-d', '--dir', type=str,
                        default="no_target", help='该次下载的数据的目录名字')
