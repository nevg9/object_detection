...
def process_content(content, test_image_id_list_temp, train_image_id_list, train_dir, test_dir, fs_dir):
    path = get_file_path(content, fs_dir)
    if content["图片id"] in test_image_id_list_temp:
...
def json_contents_to_path(actions, root_dir, test_image_id_list_temp, train_image_id_list, train_dir, test_dir, fs_dir, max_workers=3):
    process_func = partial(process_content, test_image_id_list_temp=test_image_id_list_temp, train_image_id_list=train_image_id_list,
...
def get_animal_image_train_test_data_to_local_dir(image_json_actions, image_json_name, fs_dir, root_dir, least_num_not_to_test_image_id_set,
    ...
    image_id_list = [item["图片id"] for item in image_json_actions]
